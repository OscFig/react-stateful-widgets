...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    // return setProgrammers(programmers)
    // return setProgrammers(programmers[])
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: proID > 0 ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>          
              {dev.name} <button onClick={() => { setproID(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
